...
public class Main {
	...
	*/
	...
	private static byte[][] GCMEncrypt(byte[]key,byte[]iv,byte[]input,byte[]aad) throws NoSuchAlgorithmException, NoSuchProviderException, NoSuchPaddingException, InvalidKeyException, IllegalBlockSizeException, BadPaddingException, InvalidAlgorithmParameterException{
		...
		byte[] encryptedValue = Base64.encodeBase64(encrypted);
		System.out.println("Salida guay: "+Base64.encodeBase64String(encrypted));
		//FIN GENERAR SALIDA
...
//		byte[] decryptedBytes = Base64.decodeBase64(input);
		byte[] original = cipherDecrypt.doFinal(input);
...
//		byte[]key = Base64.decode("u/Gu5posvwDsXUnV5Zaq4g==");
		byte[]key = "1234567890987654".getBytes("UTF-8");
		System.out.println("Tamaño key: "+key.length);
//		byte[] iv = Base64.decode("5D9r9ZVzEYYgha93/aUK2w==");
		byte[]iv = "7634251098463825".getBytes("UTF-8");
		System.out.println(org.bouncycastle.util.encoders.Base64.toBase64String(key));
		System.out.println(org.bouncycastle.util.encoders.Base64.toBase64String(iv));
		key = org.bouncycastle.util.encoders.Base64.decode(org.bouncycastle.util.encoders.Base64.toBase64String(key));
		iv = org.bouncycastle.util.encoders.Base64.decode(org.bouncycastle.util.encoders.Base64.toBase64String(iv));
		System.out.println("Tamaño de iv: "+iv.length);
		byte[] input = "Prueba Cifrado".getBytes("UTF-8");
		...
		try {
			...
			System.out.println("Salida: "+org.bouncycastle.util.encoders.Base64.toBase64String(encrypt[0]));
			encrypt[0] = Base64.decodeBase64("Rtn7vaMlibsGiZ/xYnbd41mFd+9PHoJfSn7FCyKB");
			byte[][] decrypt = GCMDecrypt(key, iv, encrypt[0], aad);
			System.out.println("Tamaño salida: "+decrypt[0].length);
			System.out.println("Salida: "+new String(decrypt[0],"UTF-8"));
			byte[] tagIn = encrypt[1];
			byte[] tagOut = decrypt[1];
			byte[] encryptPrueba = org.bouncycastle.util.encoders.Base64.decode("2+US4qKLwgEV1pa/O0S5zskrf85DbOuxbElTTIqP");
			byte[] tagPrueba = org.bouncycastle.util.encoders.Base64.decode("a44a8266ee1c8eb0c8b5d4cf5ae9f19a");
			boolean sonIguales = true;
			System.out.println("Tamaño encrypt: "+encrypt[0].length);
			System.out.println("Tamaño encrypt prueba: "+org.bouncycastle.util.encoders.Base64.decode("Rtn7vaMlibsGiZ/xYnbd41mFd+9PHoJfSn7FCyKB").length);
...
//				}
			...
			System.out.println("Prueba compatibilidad: ");
			String prueba = new String(org.bouncycastle.util.encoders.Base64.decode("UHJ1ZWJhIENpZnJhZG8="), "UTF-8");
			System.out.println("Prueba: "+prueba);
			System.out.println(org.bouncycastle.util.encoders.Base64.toBase64String(prueba.getBytes("UTF-8")));
